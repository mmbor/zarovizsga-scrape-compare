    ...
    with open(output_file, "w", encoding="utf-8") as out_f:
        for item in data:
            ...
            if assoc and item.get("feladatTipusId") == 3:
                ...
                # Build option lines (A, B, C, D…)
                option_lines = [f"{opt.get('betujel', '')}. {escape_tsv(opt.get('szovegPlaintext', ''))}"
                                for opt in assoc_options]

                # Find correct answer: the option whose asszociaciosLeirasTetel contains this item's id
                correct_opt = None
                for opt in assoc_options:
